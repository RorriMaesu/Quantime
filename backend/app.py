...
async def voice_chat_websocket(websocket: WebSocket):
    ...
    async def run_agent_turn():
        nonlocal assistant_speaking, audio_buffer
        if assistant_speaking or not audio_buffer:
            return
            
        assistant_speaking = True
        interrupt_event.clear()
        
        # Convert PCM to float WAV (which uses our updated 32-bit float WAV normalized generator)
        ...
        voice_prompt = "Listen to this audio and respond appropriately as the scheduling assistant."
        
        def run_stream():
            try:
                generator = generate_agent_stream(prompt=voice_prompt, chat_history=history, audio_b64=audio_b64)
        ...
        try:
            ...
            if not interrupt_event.is_set():
                ...
                update_chat_record(f"user_{chat_id}", "user", "[Voice Command]", "", "done")
